...
4. **A critic that always fails on the same kind of thing.** If the idiom critic blocks every issue
   over the same convention, the right fix is usually a line in `AGENTS.md` or an analyzer/format rule
   in the target repo — a deterministic check the gate runs once, not a judgement paid for on every
   issue.
...
(prompts/<lang>/<role>.md, gate.sh, run.sh, or the target repo's AGENTS.md / .editorconfig), and what it
